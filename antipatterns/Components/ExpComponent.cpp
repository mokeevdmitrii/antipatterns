...
int ExpComponent::GainExp(int exp_gained) {
    ...
    while (temp_level < stat_const::kMaxLevel && temp_exp < stat_const::kExpForNextLevel[temp_level]) {
...
int ExpComponent::GetCurrentExp() const {
    return current_xp_;
}

int ExpComponent::GetLevel() const {
    return level_;
}
