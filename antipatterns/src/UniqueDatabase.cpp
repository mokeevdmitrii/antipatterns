...
void UniqueDatabase::LoadUniqueData(const std::string &file_name) {
    const std::map<std::string, Json::Node> unique_data = Json::Load(file_name).GetRoot().AsMap();
...
void UniqueDatabase::LoadTextures(const std::string &file_name) {
    const std::map<std::string, Json::Node> textures = Json::Load(file_name).GetRoot().AsMap();
    for (const auto&[texture_name, texture_file_node] : textures) {
        if (!unique_data_.textures[texture_name].loadFromFile(texture_file_node.AsString())) {
            throw std::runtime_error(texture_name + "texture load error");
        }
    }
}

const UniqueData &UniqueDatabase::GetData() const {
    return unique_data_;
}
...
void UniqueDatabase::LoadEnemies(const std::string &file_name) {
    ...
    AddUniqueEnemy(EnemyType::ENEMY_SPAWNER, std::make_shared<EnemySpawner>(unique_data_.textures.at("EnemySpawner"),
                                                                            enemy_settings.at("EnemySpawner").AsMap()));

    AddUniqueEnemy(EnemyType::RAT,
                   std::make_shared<Rat>(unique_data_.textures.at("Rat"), enemy_settings.at("Rat").AsMap()));
}
...
void UniqueDatabase::LoadExits(const std::string &file_name) {
    ...
    for (const auto &exit_node : exits_nodes) {
        ...
        unique_data_.exits.try_emplace(exit_type,
                                       std::make_unique<Exit>(sf::RectangleShape(sf::Vector2f(exit_hitbox_width,
                                                                                              exit_hitbox_height)),
                                                              unique_data_.textures.at("EXITS"), curr_rect));
    }
}

void UniqueDatabase::AddUniqueEnemy(EnemyType enemy_type, std::shared_ptr<Enemy> enemy) {
    if (!unique_data_.enemies.try_emplace(enemy_type, enemy).second) {
        throw std::runtime_error(
                "You didn't load all enemies: " + std::to_string(static_cast<int>(enemy_type)) + "enemy fails");
    }
}
