...
gui::PlayerGUI::PlayerGUI(std::shared_ptr<Player> player, sf::VideoMode video_mode, const std::string &file_name) : player_(std::move(player)),
...
void gui::PlayerGUI::Update(float time_elapsed) {
    double current_hp = player_->GetAttributeValue(ATTRIBUTE_ID::CURR_HP);
    double max_hp = player_->GetAttributeValue(ATTRIBUTE_ID::MAX_HP);
    hp_bar_->Update(current_hp, max_hp);
    double current_mana = player_->GetAttributeValue(ATTRIBUTE_ID::CURR_MANA);
    double max_mana = player_->GetAttributeValue(ATTRIBUTE_ID::MAX_MANA);
    mana_bar_->Update(current_mana, max_mana);
    int current_level = player_->GetLevel();
    int current_exp = player_->GetCurrentExp();
...
void gui::PlayerGUI::InitFont() {
    gui_font_.loadFromFile(file::kMainFontFile);
}

void gui::PlayerGUI::InitHPBar(const std::map<std::string, Json::Node> &settings) {
    hp_bar_ = std::make_unique<ProgressBar>(video_mode_, settings, &gui_font_);
}

void gui::PlayerGUI::InitManaBar(const std::map<std::string, Json::Node> &settings) {
    mana_bar_ = std::make_unique<ProgressBar>(video_mode_, settings, &gui_font_);
}

void gui::PlayerGUI::InitExpBar(const std::map<std::string, Json::Node> &settings) {
    exp_bar_ = std::make_unique<ProgressBar>(video_mode_, settings, &gui_font_);
}

void gui::PlayerGUI::InitLevelBar(const std::map<std::string, Json::Node> &settings) {
