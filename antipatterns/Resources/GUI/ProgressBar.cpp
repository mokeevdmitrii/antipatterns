...
gui::ProgressBar::ProgressBar(sf::VideoMode &video_mode, const std::map<std::string, Json::Node> &settings,
                              sf::Font *font) {
    ...
    bar_params_.back.setFillColor(gui::ParseColor(settings.at("back_fill_color").AsArray()));
    bar_params_.back.setPosition(pos_x, pos_y);

    bar_params_.inner.setSize(sf::Vector2f(width, height));
    bar_params_.inner.setFillColor(gui::ParseColor(settings.at("inner_fill_color").AsArray()));
    bar_params_.inner.setPosition(pos_x, pos_y);

    if (font != nullptr) {
        bar_params_.text.setFont(*font);
        bar_params_.text.setCharacterSize(gui::CalcCharacterSize(video_mode, settings.at("character_size").AsInt()));
        bar_params_.text.setPosition(
                bar_params_.inner.getPosition().x + gui::ToPixelsX(settings.at("text_pos_x").AsFloat(), video_mode),
                bar_params_.inner.getPosition().y + gui::ToPixelsY(settings.at("text_pos_y").AsFloat(), video_mode));
    }
...
void gui::ProgressBar::Update(double current_value, double max_value) {
    ...
    float curr_percent = static_cast<float>(current_value) / static_cast<float>(bar_params_.max_value);
    ...
    bar_params_.bar_string =
            std::to_string(static_cast<int>(std::round(current_value))) + " / " + std::to_string(bar_params_.max_value);
