...
gui::SimpleBar::SimpleBar(sf::VideoMode &vm, const std::map<std::string, Json::Node> &settings) {
    ...
    text_.setCharacterSize(gui::CalcCharacterSize(vm, settings.at("character_size").AsInt()));
    text_.setPosition(back_.getPosition().x + gui::ToPixelsX(settings.at("text_pos_x").AsFloat(), vm),
            back_.getPosition().y + gui::ToPixelsY(settings.at("text_pos_y").AsFloat(), vm));

}

void gui::SimpleBar::SetFont(const sf::Font &font) {
    text_.setFont(font);
}
